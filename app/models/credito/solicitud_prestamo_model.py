...
class SolicitudPrestamo(Base):
    ...
    id_calificacion_contable_segmento = Column(
        "IDCALIFICACIONCONTABLESEGMENTO",
        Integer,
        ForeignKey("CREDITO.CALIFICACION_CONTABLE_SEGMENTO.ID"),
        nullable=False,
    )
    codigo_subcalificacion_contable = Column(
        "CODIGOSUBCALIFICACIONCONTABLE",
        NVARCHAR(30),
        ForeignKey("CREDITO.SUBCALIFICACION_CONTABLE.CODIGO"),
        nullable=False,
    )
    ...
    codigo_origen_recurso = Column(
        "CODIGOORIGENRECURSO",
        NVARCHAR(30),
        ForeignKey("CREDITO.ORIGEN_RECURSO.CODIGO"),
        nullable=False,
    )
    ...
    codigo_estado_solicitud = Column(
        "CODIGOESTADOSOLICITUD",
        NVARCHAR(50),
        ForeignKey("CREDITO.ESTADO_CREDITO.CODIGO"),
        nullable=False,
    )
    activo = Column("ACTIVO", Boolean, nullable=False)
    version = Column("VERSION", LargeBinary, nullable=True)
    codigo_asesor = Column(
        "CODIGOASESOR",
        NVARCHAR(100),
        ForeignKey("SEGURIDAD.USUARIO.USUARIO"),
        nullable=False,
    )
    codigo_destino_credito = Column(
        "CODIGODESTINOCREDITO",
        NVARCHAR(30),
        ForeignKey("CREDITO.DESTINOCREDITO.CODIGO"),
        nullable=True,
    )
