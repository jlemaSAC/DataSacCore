...
class Activo(Base):
    ...
    codigo_estado = Column(
        "CODIGOESTADO",
        NVARCHAR(50),
        ForeignKey("ACTIVOFIJO.ESTADO_ACTIVO.CODIGO"),
        nullable=False,
    )
    ...
    codigo_condicion = Column(
        "CODIGOCONDICION",
        NVARCHAR(10),
        ForeignKey("ACTIVOFIJO.CONDICION_ACTIVO.CODIGO"),
        nullable=False,
    )
    asegurado = Column("ASEGURADO", Boolean, nullable=False)
    codigo_homologado = Column("CODIGOHOMOLOGADO", NVARCHAR(100), nullable=True)
    id_area_ubicacion = Column(
        "IDAREAUBICACION",
        Integer,
        ForeignKey("ACTIVOFIJO.AREA_UBICACION.ID"),
        nullable=False,
        server_default=text("1"),
    )
    depreciacion_acumulado = Column(
        "DEPRECIACIONACUMULADO",
        Numeric(18, 2),
        nullable=False,
