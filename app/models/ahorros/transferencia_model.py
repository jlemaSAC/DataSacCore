...
class Transferencia(Base):
    ...
    id_transferencia_institucion = Column(
        "IDTRANSFERENCIAINSTITUCION",
        Integer,
    ...
    codigo_tipo_cuenta = Column(
        "CODIGOTIPOCUENTA",
        NVARCHAR(10),
        ForeignKey("AHORROS.TIPO_CUENTA_TRANSFERENCIA.CODIGO"),
        nullable=False,
    )
    cuenta = Column("CUENTA", NVARCHAR(50), nullable=False)
    codigo_concepto = Column(
        "CODIGOCONCEPTO",
        NVARCHAR(10),
        ForeignKey("AHORROS.CONCEPTO_TRANSFERENCIA.CODIGO"),
        nullable=False,
    )
